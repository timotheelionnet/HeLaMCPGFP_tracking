...
run("Cellpose Advanced", "diameter=30 cellproba_threshold=0.0 flow_threshold=0.4 "+
	"model=cyto nuclei_channel=-1 cyto_channel=1 dimensionmode=2D");

// Cellpose generates Label image, let convert them to ROIs
// it requires LaRoMe (PTBIOP update site)
print("running label to ROIs...");
run("Label image to ROIs", "");

// Use TrackMate to track the ROIs
// requires to add script to Fiji directory (PTBIOP update site)
print("running Trackmate...");
run("Run TrackMate from RoiManager", "framegap=3 linkdistance=30 "
	+"gapdistance=60.0 allowsplit=true splitdistance=80.0 ");
